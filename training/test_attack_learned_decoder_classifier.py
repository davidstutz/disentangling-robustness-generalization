...
class TestAttackLearnedDecoderClassifier:
    """
    ...
    def compute_statistics(self):
        ...
        # Correct the success by the bounds.
        upper_bound = numpy.max(self.perturbation_theta, axis=1) <= self.args.bound
        lower_bound = numpy.min(self.perturbation_theta, axis=1) >= -self.args.bound
        bound = numpy.logical_and(upper_bound, lower_bound)
        raw_overall_success = numpy.logical_and(raw_overall_success, bound[:raw_overall_success.shape[0]])
    ...
    def plot_manifolds(self):
        ...
        plot_perturbations = numpy.concatenate((self.perturbations, self.perturbation_codes), axis=1)
        ...
        for c in range(self.N_class):
            ...
            data = numpy.concatenate((
                plot_theta[:fit],
                plot_perturbations[self.perturbation_codes == c]
            ))
            classes = numpy.concatenate((
                self.test_codes[:fit],
                numpy.ones((self.perturbations[self.perturbation_codes == c].shape[0])) * 10,
            ))
...
if __name__ == '__main__':
    program = TestAttackLearnedDecoderClassifier()
    program.main()
