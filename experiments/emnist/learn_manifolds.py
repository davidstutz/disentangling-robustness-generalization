...
class LearnManifolds:
    ...
    def main(self):
        ...
        for label in labels:
            self.train('%s_%d_%d_abs_%d_%d_%d_%g_%g_%g_manual/' % (self.args.method, self.args.latent_space_size, label, self.args.epochs, self.network_channels, self.classifier_channels, self.betas[label], self.args.gamma, self.args.eta), self.args.method, label)
