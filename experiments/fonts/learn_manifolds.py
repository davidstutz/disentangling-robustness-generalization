...
class LearnManifolds:
    ...
    def __init__(self, args=None):
        ...
        paths.set_globals(experiment=self.experiment(), characters='ABCDEFGHIJ', fonts=1000, transformations=6, size=28, suffix=self.args.suffix)
    ...
    def get_parser(self):
        ...
        parser.add_argument('-beta', default=3, help='Training epochs.', type=float)
    ...
    def main(self):
        ...
        for label in labels:
            self.train('%s_%d_%d_abs_%d_%d_%d_%g_%g_%g_manual_%s/' % (self.args.method, self.args.latent_space_size, label, self.args.epochs, self.network_channels, self.classifier_channels, self.args.beta, self.args.gamma, self.args.eta, self.args.suffix), self.args.method, label)
