...
class LearnedEncoder(torch.nn.Module):
    ...
    def standard(self, resolution, **kwargs):
        ...
        code = torch.nn.Linear(representation, self.N_latent)
        #torch.nn.init.kaiming_normal_(code.weight, gain)
        torch.nn.init.normal_(code.weight, 0, 0.001)
        torch.nn.init.constant_(code.bias, 0)
        self.add_layer('code', code)
    ...
    def pool(self, resolution, **kwargs):
        ...
        code = torch.nn.Linear(representation, self.N_latent)
        #torch.nn.init.kaiming_normal_(code.weight, gain)
        torch.nn.init.normal_(code.weight, 0, 0.0005)
        torch.nn.init.constant_(code.bias, 0)
        self.add_layer('code', code)
    ...
    def mlp(self, resolution, **kwargs):
        ...
        code = torch.nn.Linear(representation, self.N_latent)
        #torch.nn.init.kaiming_normal_(code.weight, gain)
        torch.nn.init.normal_(code.weight, 0, 1./representation)
        torch.nn.init.constant_(code.bias, 0)
        self.add_layer('code', code)
    ...
    def __init__(self, N_latent, resolution=(1, 32, 32), architecture='standard', **kwargs):
        ...
        super(LearnedEncoder, self).__init__()
